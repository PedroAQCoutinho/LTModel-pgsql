...
do
    psql -v var_table=$tableName -v var_column=$columnName -v var_priority=$varPriority -U $userName -h $databaseServer -d $databaseName -p $portNumber -f proc2_get_name_result2.sql
done < saida.csv
