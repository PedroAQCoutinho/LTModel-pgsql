CREATE TABLE lt_model.result3 AS
SELECT *
FROM lt_model.result2

CREATE INDEX gix_result3 ON lt_model.result3 USING GIST (geom);

ALTER TABLE lt_model.result3
ADD CONSTRAINT pk_result3 PRIMARY KEY (gid);

CREATE INDEX gix_result3 ON lt_model.result3 USING GIST (geom);

CREATE INDEX ix_result3_1 ON lt_model.result3 USING BTREE (cd_mun);

ALTER TABLE lt_model.result3
ADD COLUMN cd_biome INT REFERENCES data.bioma_ibge (id);

CREATE INDEX ix_result3_2 ON lt_model.result3 USING BTREE (cd_biome);


UPDATE lt_model.result3 b
SET cd_biome = c.id
