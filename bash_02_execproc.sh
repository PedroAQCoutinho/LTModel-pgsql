...
psql -U $userName -h $databaseServer -d $databaseName -p $portNumber -v var_num_proc=$numProc -v var_proc=$i -f ${procName}_1.sql
...
if [ -f $4_3.sql ]; then
    psql -U $userName -h $databaseServer -d $databaseName -p $portNumber -v var_num_proc=$numProc -v var_proc=$i -f ${procName}_3.sql
fi
