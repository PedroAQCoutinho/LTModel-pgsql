...
DROP TABLE IF EXISTS recorte.proc3_22_censo_modulos;
CREATE TABLE recorte.proc3_22_censo_modulos AS
...
DROP TABLE IF EXISTS recorte.proc3_23_conhecido_tamanho;
CREATE TABLE recorte.proc3_23_conhecido_tamanho AS
...
DROP TABLE IF EXISTS recorte.proc3_24_simulado_tamanho;
CREATE TABLE recorte.proc3_24_simulado_tamanho AS
SELECT m.fk_estado_ibge cod_estado, c.cat_tamanho, COUNT(*) num_imoveis
FROM recorte.proc3_20_voronoifinal a
...
DROP TABLE IF EXISTS recorte.proc3_25_result_tamanho;
CREATE TABLE recorte.proc3_25_result_tamanho AS
...
FROM recorte.proc3_23_conhecido_tamanho a 
FULL JOIN recorte.proc3_24_simulado_tamanho b ON a.cod_estado = b.cod_estado AND a.cat_tamanho = b.cat_tamanho;


DROP TABLE IF EXISTS recorte.proc3_26_state_agg;
CREATE TABLE recorte.proc3_26_state_agg AS
...
LEFT JOIN recorte.proc3_22_censo_modulos b ON b.cod_estado = c.cod_ibge
LEFT JOIN recorte.proc3_24_simulado_tamanho d ON d.cod_estado = c.cod_ibge AND b.cat_tamanho = d.cat_tamanho
LEFT JOIN recorte.proc3_23_conhecido_tamanho e ON e.cod_estado = c.cod_ibge AND b.cat_tamanho = e.cat_tamanho
WHERE b.cod_estado = 35
GROUP BY c.cod_ibge, cat_tamanho2;

DROP TABLE IF EXISTS recorte.proc3_27_perc_final;
CREATE TABLE recorte.proc3_27_perc_final AS
...
FROM recorte.proc3_26_state_agg c;



DELETE FROM recorte.log_simulate
WHERE num_run = (SELECT MAX(num_run) FROM recorte.log_outputs);

INSERT INTO recorte.log_simulate(
            num_run, cod_state, size, num_census, num_known, num_simulated, 
            perc_census, perc_modeled)
SELECT (SELECT MAX(num_run) FROM recorte.log_outputs) num_run,
...
perc_censo,
perc_resultado
 FROM recorte.proc3_27_perc_final;