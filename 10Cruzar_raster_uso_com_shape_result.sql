CREATE TABLE lt_model.sp_fbds_count (
gid INT,
classe INT,
contagem INT
);

CREATE TABLE lt_model.sp_fbds_count2 (
gid INT,
classe INT,
contagem INT
);

CREATE INDEX ix_ras_fbds2 ON lt_model.ras_fbds2 USING BTREE ((rid % 15));

-- INSERT INTO lt_model.sp_fbds_count2 
-- SELECT b.gid, (ST_ValueCount(ST_Clip(a.rast, b.geom), 
-- 	ARRAY[15::double precision, 40, 41, 42, 80, 81, 82])).* 
-- FROM lt_model.ras_fbds2 a 
-- JOIN lt_model.result2 b ON (rid % 15) = {num} AND ST_Intersects(a.rast, b.geom);
------------------------
------ POWERSHELL ------
------------------------
-- $sql = 'INSERT INTO lt_model.sp_fbds_count2 SELECT b.gid, (ST_ValueCount(ST_Clip(a.rast, b.geom), ARRAY[15::double precision, 40, 41, 42, 80, 81, 82])).* FROM lt_model.ras_fbds2 a JOIN lt_model.result2 b ON (rid % 15) = {num} AND ST_Intersects(a.rast, b.geom);'
-- 0..14 | % {start-process cmd "/k `"echo $($_) & psql -U postgres -h geonode -d atlas -c ""$($sql -Replace '{num}', $_)`""}


SELECT COUNT(DISTINCT gid) FROM lt_model.sp_fbds_count2;

DROP TABLE lt_model.sp_fbds_count_agg;
CREATE TABLE lt_model.sp_fbds_count_agg AS
SELECT gid, classe, SUM(contagem)/400 area
FROM lt_model.sp_fbds_count
GROUP BY gid, classe;
...
FROM lt_model.sp_fbds_count_agg2 a
...
SELECT CASE WHEN classe IS NULL THEN RIGHT(b.luse_class, 2)::INTEGER ELSE classe END classe2 , SUM(CASE WHEN a.area IS NULL THEN 0 ELSE a.area END - CASE WHEN b.area_ha IS NULL THEN 0 ELSE b.area_ha END) area_diff
FROM lt_model.sp_fbds_count_agg a
FULL JOIN fa_model.sp5m_result_fbds b ON a.gid = b.prim_key AND ('Class' || a.classe) = b.luse_class
GROUP BY classe2;

DROP TABLE lt_model.ras_fbds2 
CREATE TABLE lt_model.ras_fbds (rid SERIAL, rast Raster);
