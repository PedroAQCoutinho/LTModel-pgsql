...
create table if not exists fa_model.sp5m_result_fbds_2 as
select
...
from fa_model.sp5m_result_fbds as a
group by a.prim_key;

alter table fa_model.sp5m_result_fbds_2
add column areaproc numeric;

update fa_model.sp5m_result_fbds_2
set areaproc = eq_co_at + eq_coapp1_at + eq_coapp2_at + eq_co_nv + eq_coapp1_nv + eq_coapp2_nv;

----------
create table if not exists fa_model.sp5m_result_fbds_proc0 as
    select
        a.gid as prim_key,
    ...
    join fa_model.sp5m_result_fbds_fbds_2 as c
    on a.gid = c.prim_key;

-----------
drop table if exists fa_model.sp5m_result_fbds_proc1
create table fa_model.sp5m_result_fbds_proc1 as
    select
        a.prim_key,
        a.ibgecode,
        a.biome,
        ...
        a.eq_coapp2_at,
        a.eq_coapp2_nv
    from fa_model.sp5m_result_fbds_proc0 as a
    join fa_model.sp5m_result as b
    on a.prim_key = b.gid
    
------------
drop table if exists fa_model.sp5m_result_fbds_proc2;
create table fa_model.sp5m_result_fbds_proc2 as
	select 
		a.prim_key as prim_key,
	...
	from fa_model.sp5m_result_fbds_proc1 as a;

----------
drop table if exists fa_model.temp_sp5m_result_fbds_proc3a;
create table fa_model.temp_sp5m_result_fbds_proc3a as
    select
        a.prim_key as prim_key,
...
drop table if exists fa_model.temp_sp5m_result_fbds_proc3b;
create table fa_model.temp_sp5m_result_fbds_proc3b as
    select
        a.prim_key as prim_key,
    ...
    from fa_model.sp5m_result_fbds_proc1 as a
    join fa_model.temp_sp5m_result_fbds_proc3a as b
    on a.prim_key = b.prim_key;

------------------
drop table if exists fa_model.temp_sp5m_result_fbds_proc3c;
create table fa_model.temp_sp5m_result_fbds_proc3c as
    select
        a.prim_key as prim_key,        
        case when a.ownership_classes in ('nFF', 'FF', 'TLnFF', 'TLFF') then
    ...
    from fa_model.sp5m_result_fbds_proc1 as a                  
    join fa_model.temp_sp5m_result_fbds_proc3b as b
    on a.prim_key = b.prim_key;

---------------------
drop table if exists fa_model.temp_sp5m_result_fbds_proc3d;
create table fa_model.temp_sp5m_result_fbds_proc3d as
    select
        a.prim_key as prim_key,
    ...
    join fa_model.temp_sp5m_result_fbds_proc3b as b
    on a.prim_key = b.prim_key
    join fa_model.temp_sp5m_result_fbds_proc3c as c
    on a.prim_key = c.prim_key;

 ------------------
drop table if exists fa_model.sp5m_result_fbds_proc3;
create table fa_model.sp5m_result_fbds_proc3 as 
    select
...
from fa_model.temp_sp5m_result_fbds_proc3b as b
join fa_model.temp_sp5m_result_fbds_proc3c as c
on b.prim_key = c.prim_key
join fa_model.temp_sp5m_result_fbds_proc3d as d
on b.prim_key = d.prim_key;

------------------
drop table if exists fa_model.sp5m_result_fbds_proc4;
create table fa_model.sp5m_result_fbds_proc4 as 
	select
		a.prim_key as prim_key,
	...
	from fa_model.sp5m_result_fbds_proc1 as a
	join fa_model.sp5m_result_fbds_proc2 as b
	on a.prim_key = b.prim_key
	join fa_model.sp5m_result_fbds_proc3 as c
	on a.prim_key = c.prim_key;

-------------------
drop table if exists fa_model.sp5m_result_fbds_proc5;
create table fa_model.sp5m_result_fbds_proc5 as
	select
		a.prim_key as prim_key,
        case when a.ownership_classes in ('ND', 'NP', 'PC', 'TLPC') then 0
	...
	join fa_model.sp5m_result_fbds_proc3 as c
	on a.prim_key = c.prim_key
	join fa_model.sp5m_result_fbds_proc4 as d
	on a.prim_key = d.prim_key;

----------------------------
drop table if exists fa_model.sp5m_result_fbds_processed;
create table fa_model.sp5m_result_fbds_processed as
	select
     a.prim_key as prim_key, 
	...
	join fa_model.sp5m_result_fbds_proc4 as d
	on a.prim_key = d.prim_key
	join fa_model.sp5m_result_fbds_proc5 as e
...
select
	a.*, b.geom
	from fa_model.sp5m_result_fbds_processed as a
	join fa_model.sp5m_result as b
	on a.prim_key = b.gid
...
-- Passivo por imovel
create view fa_model.sp5m_fbds_ruralpr_deficit as
	select
		a.prim_key,
		a.app_deficit,
		a.lr_deficit,
...
-- Ativo
create view fa_model.sp5m_fbds_ruralpr_ativo as
	select
		a.prim_key,
		a.not_protected,
		b.geom
